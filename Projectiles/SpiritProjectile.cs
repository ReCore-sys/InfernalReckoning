...
namespace InfernalReckoning.Projectiles
{
	...
	{
		public override string Texture => "InfernalReckoning/Items/spirit"; // Not the real texture, only added to make it compile
		...
		public override void AI()
		{
			...
			if (Distance < 50f &&
			    ...
			    Main.player[projectile.owner].position.X + Main.player[projectile.owner].width &&
			    projectile.position.X + projectile.width > Main.player[projectile.owner].position.X &&
			    projectile.position.Y <
			    Main.player[projectile.owner].position.Y + Main.player[projectile.owner].height &&
			    projectile.position.Y + projectile.height > Main.player[projectile.owner].position.Y)
			{
			...
			{
				float numX = projectile.velocity.X * 0.334f * num;
				float numY = (0f - projectile.velocity.Y * 0.334f) * num;
				float PosX = projectile.position.X + ((float) Math.Sin(a1) * 0.5f + (float) Math.Sin(360 - a2) * 0.3f +
				                                      (float) Math.Sin(a3) * 0.2f) * 16f;
				float PosY = projectile.position.Y + ((float) Math.Cos(a1) * 0.5f + (float) Math.Cos(360 - a2) * 0.3f +
				                                      (float) Math.Cos(a3) * 0.2f) * 16f;
				int DustId = Dust.NewDust(new Vector2(PosX, PosY), projectile.width, projectile.height, 182, 0f, 0f,
					100, default(Color), 1.1f);
